      ...
      <Card className="bg-white">
            ...
            <div className="flex gap-2">
              <span className="text-sm px-3 py-1 bg-green-100 text-green-800 rounded-full">
                {store.mediaSpaces.filter(space => 
                  !space.leases || 
                  (space.leases.length === 0 || 
                   (space.leases.some(lease => lease.status.id === 7) && !space.leases.some(lease => lease.status.id !== 7)))
                ).length} Disponibles
              </span>
              <span className="text-sm px-3 py-1 bg-amber-100 text-amber-800 rounded-full">
              {store.mediaSpaces.filter(space => 
                  space.leases?.some(lease => lease.status.id !== 7)
                ).length} Ocupados
              </span>
            </div>
          </div>
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {store.mediaSpaces.map((space) => {
              const activeLeases = space.leases?.filter(lease => lease.status.id !== 7).length || 0;
