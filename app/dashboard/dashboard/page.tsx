...
export default function DashboardPage() {
  ...
  const sortStores = (type: SortType) => {
    setSortBy(type);
    let sorted = [...stores];
    ...
    switch (type) {
      ...
      case 'Available':
        sorted.sort((a, b) => {
          const aAvailable = a.mediaSpaces.filter(space => space.leases?.some(lease => lease.status === 'Disponible')).length;
          const bAvailable = b.mediaSpaces.filter(space => space.leases?.some(lease => lease.status === 'Disponible')).length;
